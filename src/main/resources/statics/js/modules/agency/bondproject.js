...
var vm = new Vue({
	...
	data:{
		...
		agencys:[{agencyId:null,agencyName:null}],
		...
		fields:[
			...
			{field:'agencyName',title:'代理机构名称', width: 80 },
	...
	methods: {
		...
		getOtherInfo: function(){
			$.get(baseURL + "/bond/bondproject/getOtherInfo", function(r){
				console.log(r.BondEnagencyEntityList);
				vm.agencys = r.BondEnagencyEntityList;
				vm.user = r.BondUserEntityList;
			});
		},
		//重新加载请求
		reload: function (params) {
            $.postForm(baseURL + '/bond/bondenagency/agencySupplierPrejectList', params, function(request){
        		$("#table").bootstrapTable('load',request.page);
        	});
		},
		//新增项目的时候选择代理商
		selectAgency: function(item){
			console.log(item.target);
			console.log(item.target.value);
			vm.bondProject.agencyId = item.target.value;
		},
		//新增项目的时候选择项目负责人
		selectProjectUser: function(item){
			vm.bondProject.projectUserId = item.target.value;
		}
	},
	//创建完毕状态
    created: function(){
