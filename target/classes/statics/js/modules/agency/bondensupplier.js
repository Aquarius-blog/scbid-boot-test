...
var vm = new Vue({
	...
	data:{
		...
		bondEnsupplier: {
				...
				user:[{userId:null,userSurname:null}],
		fields:[
			...
			{field:'supplierName',title:'供应商全称', width: 80 },
			...
			//{field:'supplierBankAccount',title:'开户行', width: 80 },
			//{field:'supplierCompanyType',title:'公司类型', width: 80 },
			{field:'supplierRegAdress',title:'注册地址', width: 80 },
	...
	methods: {
        ...
        exportPage: function(){
		    ...
		    $.syncPost(baseURL + "/bond/bondensupplier/export", JSON.stringify(vm.fields), function(r){
    			if(r.code === 0){
    				alert('操作成功', function(){
		...
		getOtherInfo: function(){
			$.get(baseURL + "/bond/bondensupplier/getOtherInfo", function(r){
				vm.user = r.BondUserEntityList;
			});
		},
        //新增OR修改
		saveOrUpdate: function (event) {
			console.log("1  2 "+vm.bondEnsupplier.supplierEstabDate);
		...
		reload: function (params) {
            $.postForm(baseURL + '/bond/bondenagency/agencySupplierList',params, function(request){
        		$("#table").bootstrapTable('load',request.page);
        	});
		},
		//新增供应商的时候选择供应商监管人
		selectSupplierUser: function(item){
			console.log("dss "+item);
			console.log("ddd "+item.target.value);
			vm.bondEnsupplier.supplierUserId = item.target.value;
		}
	},
	//创建完毕状态
    created: function(){
