...
var vm = new Vue({
	...
	data:{
		...
		fields:[
			...
			{field:'guaratorName',title:'担保机构名称', width: 80 },
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		 ...
		 //审核处理
		auditOperation: function (event) {
			var url = "/bond/bondorder/auditOperation";
			$.syncPost(baseURL + url, JSON.stringify(vm.bondOrder), function(r){
        		if(r.code === 0){
        			if(vm.bondOrder.orderStatus == 2){
        				alert('操作成功，请注意查收短信', function(){
                            vm.reload();
                        });
        			}else{
        				alert('操作成功', function(){
		...
		reload: function (params) {
            $.postForm(baseURL + '/bond/bondenagency/agencysupplierOrderList', params, function(request){
		...
		//审核详情页面
		auditItemPage: function (orderId) {
        	 $('#auditModal').modal('show');
             vm.getInfo(orderId);
        },
        //审核通过
        auditTrue: function () {
        	vm.bondOrder.orderStatus = 2;
        	vm.auditOperation();
        	$('#auditModal').modal('hide');
        },
        //审核不通过
        auditFalse: function () {
        	vm.bondOrder.orderStatus = 1;
        	vm.auditOperation();
        	$('#auditModal').modal('hide');
        },
        //打开成都银行
        openCDYH: function (event){
        	var row = getSelectedRow();
        	var orderId = row.orderId;
        	applicationTable(orderId);//担保申请单
        }
	},
	//创建完毕状态
    created: function(){
