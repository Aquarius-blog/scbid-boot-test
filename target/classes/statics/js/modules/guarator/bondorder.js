 ...
 * @returns
 */
function selectGuarator(orderId){
	vm.orderIds = [];
	vm.orderIds.push(orderId);
 ...
 * @returns
 */
function acceptHandle (flag,orderId){
	var url = baseURL + "/bond/bondorder/acceptHandle?orderId="+orderId+"&flag="+flag;
	swalconfirm("确认选择操作？",function(flag){
		if(flag){
			$.get(url,function(r){
...
function uploadBond(orderId){
	...
	vm.bondOrderFile = [];
	createFilesMultiple("uploader",serverPath,function(receipt){
		console.log(receipt);
		//单个上传回调
		if (receipt.code==0) {
			vm.bondOrderFile.push(receipt.ossBean);
		}
		//全部上传完成后回调
		if(vm.bondOrderFile.length==receipt.successNum){
			var url = baseURL + "/bond/bondorder/submitUploadBond?orderId="+orderId;
			var data = {"orderId":orderId,"ossBeans":JSON.stringify(vm.bondOrderFile)}
...
var vm = new Vue({
    ...
    data : {
        ...
        fields : [
			...
			{field:'updateUesrName',title:'最后修改人',visible: false, width: 80 },
			{field:'updateDate',title:'最后修改时间',visible: false, width: 80 },
        ],
    },
    methods : {
        // 查询
